...
import { ChevronLeft, ChevronRight, ClipboardList, Plus, Save, X, Unlock, MessageCircle, Send, Pencil, Trash2, RotateCcw, ArrowLeft, ChevronDown as ChevronDownIcon, BarChart3 } from 'lucide-react';
...
function DailySummary({ tareas }: { tareas: TareaDia[] }) {
  const hechas = tareas.filter(t => t.estado === 'hecha');
  const noProcede = tareas.filter(t => t.estado === 'no procede');
  const pendientes = tareas.filter(t => t.estado === 'pendiente');
  const totalTareas = tareas.length;
  const porcentaje = totalTareas > 0 ? Math.round((hechas.length / totalTareas) * 100) : 0;

  const porPersona: Record<string, string[]> = {};
  for (const t of hechas) {
    const persona = t.hechoPor || 'Sin asignar';
    if (!porPersona[persona]) porPersona[persona] = [];
    porPersona[persona].push(t.tareaNombre);
  }

  const porTurno: Record<string, number> = {};
  for (const t of hechas) {
    porTurno[t.turno] = (porTurno[t.turno] || 0) + 1;
  }
      ...
      <CardContent className="p-4 space-y-3">
        ...
        <div className="space-y-1">
          <div className="flex justify-between text-xs text-muted-foreground">
            <span>Progreso</span>
            <span>{hechas.length}/{totalTareas} ({porcentaje}%)</span>
          </div>
          <div className="h-2.5 rounded-full bg-muted overflow-hidden">
            <div className="h-full rounded-full bg-[hsl(142,60%,45%)] transition-all duration-500" style={{ width: `${porcentaje}%` }} />
          </div>
        </div>
        <div className="grid grid-cols-3 gap-2 text-center">
          <div className="rounded-lg bg-[hsl(142,60%,90%)] dark:bg-[hsl(142,30%,18%)] p-2">
            <p className="text-lg font-bold text-[hsl(142,60%,30%)] dark:text-[hsl(142,50%,65%)]">{hechas.length}</p>
            <p className="text-[10px] text-[hsl(142,60%,30%)] dark:text-[hsl(142,50%,65%)]">Realizadas</p>
          </div>
          <div className="rounded-lg bg-[hsl(38,92%,90%)] dark:bg-[hsl(38,40%,18%)] p-2">
            <p className="text-lg font-bold text-[hsl(38,92%,30%)] dark:text-[hsl(38,80%,70%)]">{pendientes.length}</p>
            <p className="text-[10px] text-[hsl(38,92%,30%)] dark:text-[hsl(38,80%,70%)]">Pendientes</p>
          </div>
          <div className="rounded-lg bg-secondary p-2">
            <p className="text-lg font-bold text-secondary-foreground">{noProcede.length}</p>
            <p className="text-[10px] text-secondary-foreground">No procede</p>
          </div>
        </div>
        {Object.keys(porTurno).length > 0 && (
          <div>
            <p className="text-xs font-semibold text-muted-foreground mb-1">Por turno:</p>
            <div className="flex gap-2 flex-wrap">
              {Object.entries(porTurno).map(([turno, count]) => (
                <Badge key={turno} variant="outline" className="text-xs capitalize">{turno}: {count}</Badge>
              ))}
            </div>
          </div>
        )}
        {Object.keys(porPersona).length > 0 && (
          <div>
            <p className="text-xs font-semibold text-muted-foreground mb-1">Tareas realizadas por persona:</p>
            <div className="space-y-2">
              {Object.entries(porPersona).sort((a, b) => b[1].length - a[1].length).map(([persona, tareasPersona]) => (
                <div key={persona} className="rounded-lg border bg-background p-2">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-xs font-semibold">{persona}</span>
                    <Badge className={`text-[10px] border ${ESTADO_COLORS['hecha']}`} variant="outline">
                      {tareasPersona.length} tarea{tareasPersona.length !== 1 ? 's' : ''}
                    </Badge>
                  </div>
                  <div className="flex flex-wrap gap-1">
                    {tareasPersona.map((nombre, i) => (
                      <span key={i} className="text-[10px] px-1.5 py-0.5 rounded bg-muted text-muted-foreground">{nombre}</span>
                    ))}
                  </div>
                </div>
              ))}
        ...
        {hechas.length === 0 && (
          <p className="text-xs text-muted-foreground italic text-center">No hay tareas realizadas todavía.</p>
        )}
      </CardContent>
    </Card>
  );
